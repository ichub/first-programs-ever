...
class WebHandler:
  ...
  def ContainPoints(self):
    for i in range(0, len(self.points), 1):
      ...
      self.vx, self.vy = self.vel
      self.margin = 5
      if self.px < self.margin: self.px = self.margin; self.vx *= -2
      elif self.px > Simulation.screen_size_x - self.margin: self.px = Simulation.screen_size_x - self.margin; self.vx *= -2
      if self.py < self.margin: self.py = self.margin; self.vy *= -2
      elif self.py > Simulation.screen_size_y - self.margin: self.py = Simulation.screen_size_y - self.margin; self.vy *= -2
  ...
  def FollowMouse(self):
    ...
    for i in range(0, len(self.points), 1):
      ...
      if self.mouse_pressed[1]:
        self.vx += (self.mx - self.px) / self.distance * self.distance / 50
        self.vy += (self.my - self.py) / self.distance * self.distance / 50
      self.pos = self.px, self.py
      self.vel = self.vx, self.vy
      self.points[i] = self.pos, self.vel   
  def MovePoints(self):
