...
class WebHandler:
  ...
  def CreatePoints(self):
    ...
    for i in range(0, self.amount, 1):
      ...
      for j in range(0, len(self.chosen), 1):
        self.new_connection = len(self.points), self.chosen[j], random.randint(100, 150)
  ...
  def MovePoints(self):
    for i in range(0, len(self.connections), 1):
      ...
      self.px_1 += self.vx_1 * ((self.distance - self.divisor) / 5)
      self.py_1 += self.vy_1 * ((self.distance - self.divisor) / 5)
