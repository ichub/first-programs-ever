...
class PhysicsHandler:
    def __init__(self):
        self.objects = []
    def CreateObject(self, points, rotation_angle, radius, position):
        self.objects.append([])
        self.px, self.py = position
        self.vx, self.vy = random.randint(-5, 5), random.randint(-5, 5)
        self.origin = self.px, self.py, (self.vx, self.vy)
    ...
    def SimulateGravityMovement(self, index):
        ...
        for j in range(3, self.length, 1):
            ...
            self.velocity_length = math.sqrt(math.pow(self.vx, 2) + math.pow(self.vy, 2))
            if self.pox <= self.collision_border and self.vx < 0:
                self.vx *= -1
                self.vx /= 1.5
                self.px += self.collision_border - self.pox
                self.rotation_velocity += (self.py - self.poy) / 20# * (self.velocity_length / 2)
            elif self.pox >= screen_size_x - self.collision_border and self.vx > 0:
                self.vx *= -1
                self.vx /= 1.5
                self.px += screen_size_x - self.collision_border - self.pox
                self.rotation_velocity += (self.py - self.poy) / 20# * (self.velocity_length / 2)
            if self.poy <= self.collision_border and self.vy < 0:
                self.vy *= -1
                self.vy /= 1.5
                self.py = self.collision_border - self.poy
                self.rotation_velocity += (self.px - self.pox) / 20# * (self.velocity_length / 2)
            elif self.poy >= screen_size_y - self.collision_border and self.vy > 0:
                self.vy *= -1
                self.vy /= 1.5
                self.py += screen_size_y - self.collision_border - self.poy
                self.rotation_velocity = (self.px - self.pox) / 20# * (self.velocity_length / 2)
    ...
    def DrawObjects(self):
        for i in range(0, len(self.objects), 1):
            for j in range(3, len(self.objects[i]) - 1, 1):
                pygame.draw.aaline(screen, (0, 0, 0), self.objects[i][j], self.objects[i][j + 1], 3)
            pygame.draw.aaline(screen, (0, 0, 0), self.objects[i][3], self.objects[i][len(self.objects[i]) - 1], 3)
    def Update(self):
        for i in range(0, len(self.objects), 1):
            self.SimulateGravityMovement(i)
        self.DrawObjects()
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONDOWN:
            Physics.CreateObject(random.randint(3, 10), random.randint(0, 360), 100, pygame.mouse.get_pos())
    Physics.Update()
