...
class RippleHandler():
  ...
  def HandleRipples(self):
    ...
    if self.pressed[0]:
      self.ripples.append((pygame.mouse.get_pos(), 100))
  def ResizeBoxes(self):
    self.r_width = 20
    for i in range(0, len(self.ripples), 1):
      self.r_position, self.r_size = self.ripples[i]
      ...
      for j in range(0, len(self.boxes), 1):
        ...
        self.distance = math.sqrt(math.pow(self.r_px - self.b_px, 2) + math.pow(self.r_py - self.b_py, 2))
        self.ripple_distance = math.copysign(self.r_size - self.distance, 1)
        if self.ripple_distance < self.r_width:
          self.b_resize = self.b_orsize - float(self.b_orsize) / self.r_width * self.ripple_distance
          self.b_sizes = self.b_resize, self.b_resize
