...
class RippleHandler():
  ...
  def CreateRipples(self):
      self.ripples.append((pygame.mouse.get_pos(), 0, 200, 5))
  def Refresh(self):
    ...
    for i in range(0, len(self.ripples), 1):
      self.position, self.radius, self.width, self.speed = self.ripples[i]
      self.radius += self.speed
      if self.radius > 1000 + self.width: self.to_delete.append(i)
      self.ripples[i] = self.position, self.radius, self.width, self.speed
  ...
  def ResizeBoxes(self):
    for i in range(0, len(self.ripples), 1):
      self.r_position, self.r_radius, self.r_width, self.speed = self.ripples[i]
  ...
  def DrawBoxes(self):
    for i in range(0, len(self.boxes), 1):
      ...
      self.orcolor, self.recolor = self.colors
      self.orsize, self.resize = self.sizes
      self.px, self.py = self.position
      self.adjust = self.resize / 2
      self.apx = self.px - self.adjust
      self.apy = self.py - self.adjust
      #pygame.gfxdraw.rectangle(Simulation.screen,(self.apx, self.apy, self.resize, self.resize), self.recolor)
      pygame.draw.rect(Simulation.screen, self.recolor, (self.apx, self.apy, self.resize, self.resize))
  def Update(self):
    self.Refresh()
    self.ResizeBoxes()
    self.RecolorBoxes()
