...
class RippleHandler:
    ...
    def CreateBoxes(self):
        ...
        for i in range(0, screen_size_x + self.size, self.size + self.gap):
            for j in range(0, screen_size_y + self.size, self.size + self.gap):
                ...
                self.boxes.append(((i, j), (self.r, self.g, self.b), (0, 0, 0)))
    def RecolorBoxes(self):
        ...
        for i in range(0, len(self.boxes), 1):
            ...
            self.new_r = self.real_r / self.max_distance * self.distance
            self.new_g = self.real_g / self.max_distance * self.distance
            self.new_b = self.real_b / self.max_distance * self.distance
